...
-record( request_info, {
	...
	% source file) of this request method:
	%
...
-record( static_info, {
	...
	% Corresponds to the in-file location of the first defined clause (in its
	% source file) of this static method:
	%
	% (this information is not redundant with the previous field, as they are
	% different types of locations; however it is a priori redundant with the
	% one in the first clause, yet present in the forms, thus kept here; note
...
-record( class_info, {
	...
	% Include files (typically *.hrl files, but also includes the .erl source
	% module):
	%
	% (expected to remain empty, as the preprocessor is supposed to have already
	% been run)
	%
	...
	%
	% (this field must be kept synchronised with the table in the 'types' field)
	%
	type_exports :: ast_info:type_export_table(),


	% All information, indexed by type identifiers, about all the types defined
	% in that module:
	%
	types :: ast_info:type_table(),


	% All information (notably: field descriptions), indexed by record names,
	% about all the records known of that module:
	%
	...
	% All information about the class-specific (member) request methods defined
	% for that class:
	%
	% (this field must be kept synchronised with the table in the
	...
	% Whether a oneway is exported is recorded primarily in its own oneway_info
	% record through a list of locations, while the information sufficient to
	% reconstruct the actual forms for the exports of all are recorded here.
	%
	% Note: it is better that way, as a oneway export attribute may define any
	% number of exports, and we need to record its definition line.
	%
	% (this field must be kept synchronised with the table in the corresponding
	% 'oneways' field)
